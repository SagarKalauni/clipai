...
function Get-YouTubeSubtitles {
    ...
    $vttFile = Join-Path $tmpDir "sub_$($vidId).en.vtt"
    
    # Run yt-dlp to download auto subs
    & $ytdlp --write-auto-subs --sub-langs en --skip-download --output $subPrefix $videoUrl 2>&1 | Out-Null
    
    $words = @()
    if (Test-Path $vttFile) {
        $content = Get-Content $vttFile -Raw
        Remove-Item $vttFile -ErrorAction SilentlyContinue
        
        # Parse VTT content split by double newlines
        $blocks = $content -split "(?:\r?\n){2,}"
        foreach ($block in $blocks) {
            $lines = $block -split "\r?\n"
            $timeLine = $lines | Where-Object { $_ -like "*-->*" }
            if ($timeLine) {
                if ($timeLine -match "(\d{2}):(\d{2}):(\d{2})[.,](\d{3})\s*-->\s*(\d{2}):(\d{2}):(\d{2})[.,](\d{3})") {
                    ...
                    $endSec = [int]$Matches[5] * 3600 + [int]$Matches[6] * 60 + [int]$Matches[7] + [int]$Matches[8] / 1000.0
                    
                    $textLines = @()
                    $idx = $lines.IndexOf($timeLine) + 1
                    while ($idx -lt $lines.Length) {
                        $textLines += $lines[$idx]
                        $idx++
                    }
                    $cleanText = ($textLines -join " " -replace "<[^>]+>", "").Trim()
                    $splitWords = $cleanText -split "\s+"
                    $duration = $endSec - $startSec
                    $wordDur = $duration / [Math]::Max(1, $splitWords.Length)
                    
                    for ($wIdx = 0; $wIdx -lt $splitWords.Length; $wIdx++) {
